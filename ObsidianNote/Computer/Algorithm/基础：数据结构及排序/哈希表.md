...
| 函数 / 方法                       | 作用描述                   | 参数                  | 示例调用                               |
| ----------------------------- | ---------------------- | ------------------- | ---------------------------------- |
| `unordered_map<Key,Value> m;` | 创建哈希表                  | 无                   | `unordered_map<int, string> m;`    |
| `m[key] = value;`             | 插入或更新键值对               | 键 `key`，值 `value`   | `m[1] = "apple";`                  |
| `m.insert({key, value});`     | 插入键值对（不覆盖已有key）        | pair `{key, value}` | `m.insert({2, "banana"});`         |
| `m.erase(key);`               | 删除指定键的元素               | 键 `key`             | `m.erase(2);`                      |
| `m.find(key);`                | 查找元素                   | 键 `key`             | `auto it = m.find(1);`             |
| `m.count(key);`               | 判断键是否存在                | 键 `key`             | `if(m.count(1)) cout << "exists";` |
| `m.size();`                   | 返回元素个数                 | 无                   | `cout << m.size();`                |
| `m.clear();`                  | 清空哈希表                  | 无                   | `m.clear();`                       |
| `m.empty();`                  | 判断是否为空                 | 无                   | `if(m.empty()) ...`                |
| `m[key]`                      | 访问键对应的值，如果键不存在会自动插入默认值 | 键 `key`             | `string val = m[3];`               |
| `unordered_set<Key> s;`       | 创建哈希集合                 | 无                   | `unordered_set<int> s;`            |
| `s.insert(key);`              | 插入元素                   | 键 `key`             | `s.insert(5);`                     |
| `s.erase(key);`               | 删除元素                   | 键 `key`             | `s.erase(5);`                      |
| `s.find(key);`                | 查找元素                   | 键 `key`             | `auto it = s.find(5);`             |
| `s.count(key);`               | 判断元素是否存在               | 键 `key`             | `if(s.count(5)) cout << "found";`  |
| `s.size();`                   | 返回元素个数                 | 无                   | `cout << s.size();`                |
| `s.clear();`                  | 清空集合                   | 无                   | `s.clear();`                       |
| `s.empty();`                  | 判断是否为空                 | 无                   | `if(s.empty()) ...`                |